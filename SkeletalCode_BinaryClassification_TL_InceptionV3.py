...
opt='adam'
tf_lrs = 0.15    #learning rate for fitting classifier
tune_lrs = 0.015    #learning rate for fine tunning
decays = 0.01
bs = 32    #batch size
nb_epochs = 20
...
# Learning
es = EarlyStopping(monitor='val_binary_accuracy', mode='max', verbose=1, patience=5, min_delta=0.001, restore_best_weights=True)
checkpoint = ModelCheckpoint('/weghits/tf_inceptionV3-{epoch:03d}.h5', verbose=1, monitor='val_binary_accuracy', save_best_only=True, mode='max')
...
summarize_diagnostics(lr_history, file_name="gray_Learning_Curve_tf")
...
checkpoint = ModelCheckpoint('/weghits/tf_inceptionV3-{epoch:03d}.h5', verbose=1, monitor='val_binary_accuracy', save_best_only=True, mode='max')
lr_history = model.fit(train_it, steps_per_epoch=nb_train_samples // bs, validation_data=val_it, validation_steps=nb_val_samples // bs, epochs=nb_epochs, verbose=1, callbacks=[es, checkpoint])
summarize_diagnostics(lr_history, file_name="gray_Learning_Curve_finetuning")
